...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['profileScope'] ?? '') === 'professionista')) {
  ...
  try {
    $emailNormalized = mb_strtolower($emailInput, 'UTF-8');

    $duplicate = Database::exec(
      'SELECT idUtente FROM Utenti
       WHERE idUtente <> ? AND (email = ? OR emailNormalizzata = ?)
       LIMIT 1',
      [$userId, $emailInput, $emailNormalized]
    )->fetch();

    if ($duplicate) {
      http_response_code(409);
      echo json_encode(['ok' => false, 'message' => 'Email già in uso da un altro account.']);
      exit;
    }

    Database::exec(
      'UPDATE Utenti
       SET nome = ?, cognome = ?, email = ?, emailNormalizzata = ?, aggiornatoIl = NOW()
       WHERE idUtente = ?',
      [$nomeInput, $cognomeInput, $emailInput, $emailNormalized, $userId]
    );
...
function renderStart(string $title, string $activeTab, string $email, string $roleBadge, bool $isPt, bool $isNutrizionista): void {
  ...
  echo '.nav{display:flex;align-items:center;justify-content:space-between;min-height:74px;gap:12px}.brand{display:inline-flex;align-items:center;font-weight:700;letter-spacing:.05em;font-size:18px;color:#f5f8ff;padding:8px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.03);box-shadow:0 10px 28px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.06)}';
...
function renderEnd(string $scripts = ''): void {
  ...
  $tabs = [
    ...
    'allenamenti' => ['label' => 'Allenamenti', 'href' => 'allenamenti.php', 'visible' => $isPt],
    'nutrizione' => ['label' => 'Nutrizizione', 'href' => 'nutrizione.php', 'visible' => $isNutrizionista],
    'questionari' => ['label' => 'Questionari', 'href' => 'questionari.php', 'visible' => true],
