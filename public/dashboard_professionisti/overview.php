...
if (!empty($isPt) && !empty($isNutrizionista)) {
  $roleLabel = 'PT e Nutrizionista';
} elseif (!empty($isPt)) {
  $roleLabel = 'PT';
} elseif (!empty($isNutrizionista)) {
...
<section><div class="kpi-wrap-title"><h3>Panoramica</h3><a class="btn-lite" href="report.php">Dettagli</a></div><div class="kpi-grid"><?php foreach ($kpiItems as $kpi): ?><?php if (!empty($kpi['href'])): ?><a class="kpi-link" href="<?= h((string)$kpi['href']) ?>"><?php endif; ?><article class="kpi-card acc-<?= h($kpi['accent']) ?>"><span class="kpi-icon"><?= $kpi['accent'] === 'info' ? '🔑' : ($kpi['accent'] === 'success' ? '⭐' : ($kpi['accent'] === 'warning' ? '🗓️' : '👥')) ?></span><p class="kpi-label"><?= h($kpi['label']) ?></p><p class="kpi-value"><?= h($kpi['value']) ?></p><p class="kpi-helper"><?= h($kpi['helper']) ?></p></article><?php if (!empty($kpi['href'])): ?></a><?php endif; ?><?php endforeach; ?></div></section>
